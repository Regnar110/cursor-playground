...
describe('set + get', () => {
    ...
    test('Redis payload includes _meta for Redis Insight debugging', async () => {
        await handler.set(CACHE_KEY, Promise.resolve(makeEntry()));
        const buf = FakeRedis.state.store.get(ENCODED_KEY);
        if (!buf) return;

        const raw = v8.deserialize(buf) as {
            _meta: { layer: string; resource: string; locale: string };
        };
        expect(raw._meta).toMatchObject({ layer: 'data', locale: 'pl/pl', resource: 'posts' });
    });
    ...
    test('entry past revalidate but before expire is still served (SWR)', async () => {
        await handler.set(
            CACHE_KEY,
            Promise.resolve(
    ...
    test('entry past expire window is skipped (miss)', async () => {
        await handler.set(
            CACHE_KEY,
